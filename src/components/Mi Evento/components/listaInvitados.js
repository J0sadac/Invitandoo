        ...
        <div className="lista card-body">
           ...
           {invitados.map((datos, index) => (
                <div className='invitado' key={index}>
                    ...
                    <button className={`boton copiar ${copiado[index] ? 'copiado' : ''}`} onClick={() => confirmar(index)}>
                        <CopyToClipboard text={`https://invitandoo.com/evento/${eventoId}/invitado/${datos._id}`}>
                            {copiado[index] ? (
                    ...
                    <button className='boton eliminar' onClick={() => eliminarInvitado(datos._id)}>
